...
- 🎯 **X Mentions & Replies** - Intent recognition with safe, contextual responses
...
- **X API** (OAuth 1.0a recommended)
  ...
  - **Mentions & Replies**: Fetches mentions every 2 minutes, responds with AI-generated contextual replies
- **Moltbook** (skill-spec driven)
  ...
  - **Threaded Comment Replies** (✅ FIXED): Fetches comments via `/agents/profile` + `/posts/{id}`, generates GPT-4o-mini replies, posts to `/posts/{postId}/comments` with `parent_id` for proper threading, respects 20s cooldown
- **Multi-target fanout**: `SOCIAL_MODE=multi` posts to X + Moltbook in sequence with independent failure isolation
- **AI-Powered Engagement**: All replies (mentions, comments) generated by GPT-4o-mini with Based Intern personality
